...
namespace Hooks
{
	...
	void* ProcessEventDetour(UObject* pObject, UObject* pFunction, void* pParams)
	{
		if (pObject && pFunction) {
			...
			{
				printf("pressed play!\n");
				Funcs::SwitchLevel(Globals::LocalPlayerController, L"DS_Buildergridplane?Game=Athena");
				bIsReady = true;
			}
			...
			if (pFunction->GetName().find("ReadyToStartMatch") != std::string::npos && bIsReady)
			{
				if (!bHasSpawned) {
					...
					Globals::GameState = UObject::FindObject("Athena_GameState_C DS_BuilderGridPlane.DS_BuilderGridPlane.PersistentLevel.Athena_GameState_C");
					Globals::GameMode = UObject::FindObject("Athena_GameMode_C DS_BuilderGridPlane.DS_BuilderGridPlane.PersistentLevel.Athena_GameMode_C");
					...
					Globals::LocalPawn = SpawnActor(UObject::FindObject("BlueprintGeneratedClass PlayerPawn_Athena.PlayerPawn_Athena_C"), FVector(0, 0, 1500), FRotator());
